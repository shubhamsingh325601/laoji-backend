...
// The Vendor app's add-product form, one per business type. Served by
// GET /vendor/catalog/product-form and rendered field by field by the app,
// so a form can change with a backend deploy instead of an app release.
//
// Keys `name`, `brand`, `unit` and `size` are products columns and a
// `category` field is the category picker; every other key is stored in
// products.attributes. Photo and pricing/stock are the same for every type
// and stay fixed in the app — `showMrp` only toggles the MRP input there.
...
type FormDefinition = Omit<ProductForm, 'businessType'>;

const COLUMN_KEYS = new Set(['name', 'brand', 'unit', 'size']);

const CATEGORY: ProductFormField = { key: 'category', label: 'Category', type: 'category', required: true };

const GROCERY: FormDefinition = {
  basic: [
...
export function productFormFor(businessType: string): ProductForm {
  if (businessType === 'restaurant') {
    throw new BadRequestException('Restaurants add menu items instead of products');
  }
...
export function readProductAttributes(
  ...
  for (const field of [...form.basic, ...form.details]) {
    ...
    if (!COLUMN_KEYS.has(field.key)) attributes[field.key] = value as string | number | boolean;
  }
  return attributes;
}

function isValidValue(field: ProductFormField, value: unknown): boolean {
  switch (field.type) {
    case 'text':
